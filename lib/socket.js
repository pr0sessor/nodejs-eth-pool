...
io.on('connection', (socket) => {
  ...
  socket.on('candidate', (data, key) => {
    if (key !== config.socket.key) return
    Round.find({ createdAt: { $gt: new Date(Date.now() - (config.unlocker.hashrateDuration * 1000)) } }, (err, res) => {
      if (err) return logger('error', 'mongo', err.message)
      const tmpRound = []
      res.forEach((share) => {
  ...
  socket.on('unlock_update', (data, key) => {
    if (key !== config.socket.key) return
    const { number } = data
    Reward.updateMany({ number }, { status: 'pending' })
  })
  socket.on('fee_update', (data, key) => {
    if (key !== config.socket.key) return
