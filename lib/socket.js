...
io.on('connection', (socket) => {
  ...
  socket.on('share', (data) => {
    ...
    Miner.findOneAndUpdate({ address: data.address, workerName: data.workerName }, { lastShare: moment().unix() })
  })
  ...
  socket.on('unlock_update', data => {
    const { address, immature, pending } = data
    Balance.findOne({ address }, (err, balance) => {
      if (err) return logger('error', 'mongo', err.message)
      Object.assign(balance, {
        immature: Number(balance.immature) - immature,
        pending: Number(balance.pending) + pending
      }).save()
    })
  })
