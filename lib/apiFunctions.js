...
const getMinerStats = async (address, cb) => {
  ...
  // Blocks
  const blockCount = await Block.countDocuments({ address, status: { $ne: 'orphan' } })
  if (blockCount) {
  ...
  // Payments
  const dbPayment = await Payout.find({ address, status: 'paid' }, null, { sort: { datePaid: -1 }, limit: 50 })
  if (dbPayment) {
...
const getPoolBlocks = async (cb) => {
  ...
  let blocks = await Block.find({ status: { $ne: 'orphan' } }, null, { sort: { createdAt: -1 }, limit: (50 - candidates.length) })
  ...
  const immature = await Block.countDocuments({ status: 'immature' })
  const unlocked = await Block.countDocuments({ status: 'unlocked' })
...
const getPoolPayments = async (cb) => {
  const payments = await Payout.find({ status: 'paid' }, null, { sort: { datePaid: -1 }, limit: 50 })
  cb(payments)
}
