...
app.get('/api/accounts/:address', catchAsync(async (req, res) => {
  ...
  if (dbMiner) {
    stats.miners = await Promise.all(dbMiner.map(async miner => {
      const dbShare = await Hashrate.find({ createdAt: { $gt: moment().unix() - 600 }, address, workerName: miner.workerName })
      let totalShares = 0
      if (dbShare) {
        dbShare.forEach(share => {
          totalShares += Number(share.difficulty)
    ...
    list = await Promise.all(users.map(async user => {
      ...
      if (dbMiner) {
        const tempMiners = await Promise.all(dbMiner.map(async miner => {
          const dbShare = await Hashrate.find({ createdAt: { $gt: moment().unix() - 600 }, address: user.address, workerName: miner.workerName })
          let totalShares = 0
          if (dbShare) {
            dbShare.forEach(share => {
              totalShares += Number(share.difficulty)
