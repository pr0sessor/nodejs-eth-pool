...
app.get('/api/stats', catchAsync(async (req, res) => {
  ...
  // pool hashrate
  const dbShare = await Hashrate.find({ createdAt: { $gt: moment().unix() - 600 } })
  if (dbShare) {
    let totalShares = 0
  ...
  if (block) {
    stats.lastBlockFound = block.createdAt
  }
  res.json(stats).end()
}))
...
app.get('/api/accounts/:address', catchAsync(async (req, res) => {
  ...
  // Hashrate
  const dbShare = await Hashrate.find({ createdAt: { $gt: moment().unix() - 600 }, address })
  if (dbShare) {
    let totalShares = 0
    dbShare.forEach(share => {
      totalShares += Number(share.difficulty)
  ...
  if (dbMiner) {
    stats.miners = await Promise.all(dbMiner.map(async miner => {
      const dbShare = await Hashrate.find({ createdAt: { $gt: moment().unix() - 600 }, address, worker: miner.name })
      let totalShares = 0
      if (dbShare) {
        dbShare.forEach(share => {
          totalShares += Number(share.difficulty)
      ...
      return {
        ...
        shares: miner.shares,
        name: miner.name,
        lastShare: moment(miner.lastShare).unix()
      }
    }))
  }
    ...
    list = await Promise.all(users.map(async user => {
      ...
      if (dbMiner) {
        const tempMiners = await Promise.all(dbMiner.map(async miner => {
          const dbShare = await Hashrate.find({ createdAt: { $gt: moment().unix() - 600 }, address: user.address, worker: miner.name })
          let totalShares = 0
          if (dbShare) {
            dbShare.forEach(share => {
              totalShares += Number(share.difficulty)
          ...
          return {
            hashrate: parseInt(totalShares / 600),
            lastShare: (miner.lastShare ? miner.lastShare : null),
            status: miner.status
          }
        }))
