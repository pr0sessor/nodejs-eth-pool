...
const { Miner, Balance, Block, Hashrate, Payout, Candidate } = require('../models')
...
app.get('/api/accounts/:address', catchAsync(async (req, res) => {
  ...
  stats.immature = bignum(dbUser.immature).toString()
  stats.pending = bignum(dbUser.pending).toString()
  stats.paid = bignum(dbUser.paid).toString()
