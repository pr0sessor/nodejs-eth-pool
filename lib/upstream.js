...
class Upstream {
  ...
  getWork (cb) {
    rpc('eth_getWork', [], (err, work) => {
      if (err) {
        console.log(err)
        return logger('error', 'upstream', 'Failed to get work')
      }
      this.setJob(work)
      cb(work)
    })
  ...
  submitWork (nonce, powHash, mixHash, height, miner) {
    rpc('eth_submitWork', [nonce, powHash, mixHash], (err, result) => {
      ...
      if (result) {
        ...
        logger('error', 'stratum', `Rejected Block #${height} was submitted by ${miner.address}@${miner.ip}`)
      } */
    })
  }
  ...
  processEPShare (params, miner, ethash, sendReply) {
    ...
    socket.emit('share', {
      ...
      difficulty: miner.difficulty * config.diffToShare,
      solo: miner.solo
    }, config.socket.key)
    logger('success', 'stratum', `Valid share received from ${miner.address}@${miner.ip}`)
    console.log(parseInt(utils.preHex(r.result.toString('hex'))), parseInt(job.blockTarget))
    if (parseInt(utils.preHex(r.result.toString('hex'))) > parseInt(job.blockTarget)) {
      this.submitWork(params[0], utils.preHex(job.powHash), r.mix_hash, job.blockHeight, miner)
    }
    sendReply(null, true)
  }

  processESShare (params, miner, ethash, sendReply) {
    if (!params || params.length !== 3) return sendReply('Malformed PoW result', null)
    if (!miner) return sendReply('Not subscribed', null)
    if (!ethash) return sendReply('Validator is not yet ready', null)
    ...
    logger('success', 'stratum', `Valid share received from ${miner.address}@${miner.ip}`)
    if (bignum(utils.preHex(r.result.toString('hex'))) > bignum(job.blockTarget)) {
