...
function handleMinerData (method, params, socket, portData, difficulty, sendReply, sendReplyNH, pushMessage, makeJob, makeJobNH) {
  ...
  methods.eth_submitLogin = () => {
    ...
    const miner = new Miner(utils.generateUnid(), socket, address, pass, solo, false, workerName, socket.remoteAddress, portData, difficulty, pushMessage)
  ...
  methods['mining.authorize'] = () => {
    ...
    const miner = new Miner(utils.generateUnid(), socket, address, pass, solo, true, workerName, socket.remoteAddress, portData, difficulty, pushMessage)
    miners[socket.extraNonce] = miner
