...
function handleMinerData (method, params, socket, portData, difficulty, sendReply, sendReplyNH, pushMessage, makeJob, makeJobNH) {
  ...
  methods.eth_submitLogin = () => {
    ...
    if (!utils.validateAddress(login)) return sendReply('Invalid address')
    if (!workerName) workerName = 0

    const miner = new Miner(utils.generateUnid(), socket, login, pass, solo, false, workerName, socket.remoteAddress, portData, difficulty, pushMessage)
  ...
  methods['mining.authorize'] = () => {
    ...
    if (!utils.validateAddress(login)) return sendReplyNH('Invalid address')
    if (!workerName) workerName = 0

    const miner = new Miner(utils.generateUnid(), socket, login, pass, solo, true, workerName, socket.remoteAddress, portData, difficulty, pushMessage)
    miners[socket.extraNonce] = miner
...
function StratumServer (portData) {
  const makeJob = function (extraNonce) {
    ...
    return [
      topJob.powHash,
      topJob.seedHash,
      utils.diffToTarget(miner.difficulty)
    ]
  }
